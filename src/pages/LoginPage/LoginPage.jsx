...
const LoginPage = ({
  ...
  const handleSwitchToSignup = () => {
    if (isLoading) return; // prevent switching during login
  ...
  useEffect(() => {
    if (isLoading) return; // wait till loading finishes

    if (loginData && Object.keys(loginData).length && !error) {
      toast({
        title: "Login successful",
        status: "success",
        duration: 3000,
  ...
  }, [loginData, error, isLoading]);

  return (
    <main className="bg-black/90 fixed inset-0 flex justify-center items-center z-10">
      <div className="bg-white flex flex-col p-6 rounded-lg w-1/2 max-w-md max-[500px]:w-[90%]">
        <header className="flex justify-between items-center mb-6">
          <span className="text-3xl text-gray-700 font-medium">Login</span>
          <ImCross
            onClick={() => !isLoading && setShowLoginPage(false)}
            className="cursor-pointer text-black"
          />
        </header>

        <form className="flex flex-col gap-4 pb-8 mb-2 border-b" onSubmit={handleSubmit}>
          <input
            type="email"
            name="email"
            placeholder="Email"
            value={loginDetails.email}
            onChange={handleInputChange}
            className="outline-none border text-gray-600 rounded-lg p-2"
          />

          <div className="flex items-center justify-between p-2 rounded-lg border">
            <input
              type={loginDetails.passwordVisibility ? "text" : "password"}
              name="password"
              placeholder="Enter Password"
              value={loginDetails.password}
              onChange={handleInputChange}
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="bg-primary text-white p-2 rounded-lg flex justify-center"
          >
